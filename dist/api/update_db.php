...
try {
    $stmt = $pdo->prepare("UPDATE profile SET address=?, addressShort=?, bio=?, experience='' WHERE id=1");
    $stmt->execute([$address, $addressShort, $bio]);
    echo "<h1>Sucesso!</h1><p>Endereço, Bio e Experiência atualizados no banco de dados.</p>";
} catch (Exception $e) {
